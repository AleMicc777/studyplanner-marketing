...
≥4 distinct layout families; no page of identical rectangles; cards only where genuinely best, never nested. Preserve anchor IDs: `#home`, `#problem`, `#how-it-works`, `#demo`, `#universities`, `#dashboard`, `#faq`, `#contact`. The AdminDashboard section (owner-commissioned 2026-08-16) carries the three real admin screenshots in BrowserFrames plus the "statistics gathered" ledger list; its numbers come from the screenshots themselves (568 modules) or stay qualitative. The mobile sticky CTA bar (StickyMobileCTA: graphite, 44px dismiss, session-remembered) is owner-approved shipped UI and part of the system.
