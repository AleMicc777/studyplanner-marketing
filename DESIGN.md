...
Exactly 4 sitewide, one stroke system: 96-unit viewBox, stroke 1.75 `vector-effect: non-scaling-stroke`, round caps/joins, squared bends radius 2 (drafted, not doodled), ink stroke, ONE accent pop each (one teal filled shape OR one teal stroke, never both). Dashes only inside an illustration, one detail ≤6 dashes. 160-200px desktop, 120px at 390. Motifs: Problem = plan sheet gone wrong (teal-circled EC total); HowItWorks = rule stamp with teal check imprint; FoundingPilot = paper plane folded from plan sheet (teal fold, dashed trail = the sanctioned dash); TrustSecurity = closed ledger with clasp and teal bookmark.
...
≥4 distinct layout families; no page of identical rectangles; cards only where genuinely best, never nested. Preserve anchor IDs: `#home`, `#problem`, `#how-it-works`, `#demo`, `#universities`, `#faq`, `#contact`. The mobile sticky CTA bar (StickyMobileCTA: graphite, 44px dismiss, session-remembered) is owner-approved shipped UI and part of the system.
